...
use crate::error::ValidityCheckError;
    ...
    match (eth_response, orchestrator_response) {
        (Ok(e), Ok(o)) => {
                ...
                && req_delegate_orchestrator_address != delegate_orchestrator_address
            {
                ...
                Err(ValidityCheckError::EthreumOrchestratorAddressesIncorrect {
                    delegate_eth_address,
                    req_delegate_eth_address,
                    delegate_orchestrator_address,
                    req_delegate_orchestrator_address,
                })
                ...
                Err(ValidityCheckError::EthereumAddressIncorrect {
                    delegate_eth_address,
                    req_delegate_eth_address,
                })
                ...
                Err(ValidityCheckError::OrchestratorAddressIncorrect {
                    delegate_orchestrator_address,
                    req_delegate_orchestrator_address,
                })
                ...
                Err(ValidityCheckError::DifferentValidtatorAddresses)
            } else {
                Ok(())
            }
        }
        (Err(e), Ok(_)) => {
            ...
            Err(ValidityCheckError::EthereumAddressPrivateKeyIncorrect(e))
        }
        (Ok(_), Err(e)) => {
            ...
            Err(ValidityCheckError::OrchestratorAddressPharseIncorrect(e))
        }
        (Err(_), Err(_)) => {
            error!("Delegate keys are not set! Please Register your delegate keys");
            Err(ValidityCheckError::DelegateKeysNotSet)
        }
    }
}
...
pub async fn check_for_fee(
    ...
    for balance in balances {
        if balance.denom.contains(&fee.denom) {
            if balance.amount < fee.amount {
                ...
                return Err(ValidityCheckError::FeeGreaterThanBalance);
            } else {
                return Ok(());
            }
        }
    ...
    Err(ValidityCheckError::NoBalanceForSpecifiedToken {
...
pub async fn check_for_eth(address: EthAddress, web3: &Web3) -> Result<(), ValidityCheckError> {
    ...
    if balance == 0u8.into() {
        ...
        Err(ValidityCheckError::EthereumBalanceZero)
    } else {
        Ok(())
    }
}
