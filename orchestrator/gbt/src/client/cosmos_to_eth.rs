...
pub async fn cosmos_to_eth(args: CosmosToEthOpts, address_prefix: String) -> Result<()> {
    ...
    match res {
        ...
        Err(_e) => {
            ...
            return Err(anyhow!(
                "Asset {} has no ERC20 representation",
                gravity_coin.denom
            ));
        }
    }
    ...
    if found.is_none() {
        ...
        return Err(anyhow!("You don't have any {} tokens!", gravity_coin.denom));
        ...
        return Err(anyhow!(
            "You transfer of tokens is greater than your balance!"
        ));
    }
