...
pub async fn deploy_erc20_representation(
    ...
    let contract_address = if let Some(c) = args.gravity_contract_address {
        ...
        if c.is_err() {
            ...
            return Err(anyhow!(
                "The Gravity address is not yet set as a chain parameter! You must specify --gravity-contract-address"
            ));
        }
        c.unwrap()
    };
    ...
    if let Ok(val) = res {
        ...
        info!("Asset {} already has ERC20 representation {}", denom, erc20);
        return Err(anyhow!(
